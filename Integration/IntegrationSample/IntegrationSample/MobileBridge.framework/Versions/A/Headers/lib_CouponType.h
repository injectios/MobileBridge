...
//	NSString*  code;  // activation code
    ...
    int extra360ViewId; // if not exist then -1
    int extraGameId; // if not exist then -1
    NSDictionary* extraCodeCollection; // if not exist then nil;
...
//    BOOL auto_save;
    NSArray* tags;
    int loyalty_cost; // number of points for redemtions
...
@property (nonatomic) int extra360ViewId;
@property (nonatomic) int extraGameId;
@property (nonatomic, retain) NSDictionary* extraCodeCollection;
...
//@property (nonatomic) BOOL auto_save;
@property (nonatomic, strong) NSArray* tags;
@property (nonatomic) int loyalty_cost;

@property (nonatomic) int status;


-(id)initWithValue:(NSDictionary*)pCouponInfo template:(BOOL)template command:(int)command;
-(void)description;
